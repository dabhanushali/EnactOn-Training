    ...
    try {
      ...
      if (crawlResponse?.data?.length > 0) {
        const content = crawlResponse.data
          ...
          .join("\n\n---\n\n");
        console.log(
          `Successfully scraped ${crawlResponse.data.length} pages with Firecrawl`
        );
        return content;
      }
      ...
      console.log(
        "Firecrawl scraping failed, attempting basic fetch:",
        error.message
      );
    }
  }
...
- If content mentions structures like "Month 1, Week 1", ALWAYS create modules for the weekly structure (e.g., "Module 1: Week 1"). Prioritize weeks over months.`;
  ...
  try {
    ...
    console.log(
      "Primary AI call failed or returned empty. Status:",
      response.status
    );
  } catch (error) {
 ...
 */ function validateAndCleanData(rawResponse) {
  ...
  if (!parsed.course || !Array.isArray(parsed.modules)) {
    throw new Error(
      'AI response missing required "course" or "modules" structure'
    );
  }
  const course = {
    course_name: String(parsed.course.course_name || "Generated Course")
      .trim()
      .slice(0, 200),
    course_description: String(
      parsed.course.course_description || "Course generated from content"
    )
      .trim()
      .slice(0, 500),
    course_type: [
      ...
      "Leadership",
      "Other",
    ].includes(parsed.course.course_type)
      ? parsed.course.course_type
      : "Technical",
    difficulty_level: ["Beginner", "Intermediate", "Advanced"].includes(
      parsed.course.difficulty_level
    )
      ? parsed.course.difficulty_level
      : "Intermediate",
    target_role: String(parsed.course.target_role || "")
      .trim()
      .slice(0, 200),
    learning_objectives: String(parsed.course.learning_objectives || "")
      .trim()
      .slice(0, 500),
  };
  ...
  const processContentItems = (items) => {
    return items
      ...
      .map((item) => {
        ...
        const validTypes = ["External Link", "Video", "PDF", "Text", "Project"];
        if (!validTypes.includes(contentType)) {
          contentType = contentUrl ? "External Link" : "Text";
        }
        const durationNum =
          parseInt(String(item.estimated_duration_minutes), 10) || 45;
        return {
  ...
  const modules = parsed.modules
    .filter(
      (m) =>
        m && typeof m === "object" && m.module_name && Array.isArray(m.contents)
    )
    .slice(0, 20)
    .map((mod, index) => {
      ...
      if (processedContents.length === 0 && mod.contents.length > 0) {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
          console.log(
            `--- WARNING: Module "${
              mod.module_name
            }" produced empty content. Retrying (${i + 1}/${MAX_RETRIES})...`
          );
          ...
          if (processedContents.length > 0) {
            console.log(
              `--- SUCCESS: Retry successful for module "${mod.module_name}".`
            );
            break;
          }
        }
      ...
      if (processedContents.length === 0 && mod.contents.length > 0) {
        console.error(
          `--- CRITICAL FAILURE: Could not process content for module "${mod.module_name}" after all retries.`
        );
        console.error(
          "--- PROBLEMATIC ORIGINAL CONTENT:",
          JSON.stringify(mod.contents, null, 2)
        );
      }
      // --- END: RETRY LOGIC (CORRECTED) ---
      return {
        module_name: String(mod.module_name || `Module ${index + 1}`)
          .trim()
          .slice(0, 100),
        module_description: String(mod.module_description || "Module content")
          .trim()
          .slice(0, 300),
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Successfully generated course "${cleanedData.course.course_name}" with ${cleanedData.modules.length} modules.`
    );
